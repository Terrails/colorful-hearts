...
//        RenderGuiLayerEvents.before(RenderGuiLayerEvents.PLAYER_HEALTH).register((mc, guiGraphics, deltaTracker) -> {
//            if (mc.getCameraEntity() instanceof Player player) {
//                int health = Mth.ceil(player.getHealth());
//                float maxHealth = Math.max((float)player.getAttributeValue(Attributes.MAX_HEALTH), (float)Math.max(mc.gui.displayHealth, health));
